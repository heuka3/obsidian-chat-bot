...
function createTurndownService() {
    ...
    function mdFromCell(cell) {
        // 셀 내부의 마크업을 turndown으로 변환 (링크/강조/코드 등 보존)
        let md = turndownService.turndown(cell.innerHTML || '').trim();
        // 불필요한 UI 요소 제거
    ...
    function getAlign(cell) {
        ...
        const align = (alignAttr || (match ? match[1] : ''));
        if (align === 'center')
            return 'center';
        if (align === 'right')
            return 'right';
    ...
    function tableToGrid(table) {
        ...
        trs.forEach((tr, rIndex) => {
            ...
            let cIndex = 0;
            // 이미 채워진 칸 건너뛰기
            while (grid[rIndex][cIndex] !== undefined)
                cIndex++;
            const cells = Array.from(tr.children).filter(td => (td.nodeName === 'TD' || td.nodeName === 'TH') && !isHidden(td));
            cells.forEach(td => {
                // 현재 행에서 비어있는 다음 칸 찾기
                while (grid[rIndex][cIndex] !== undefined)
                    cIndex++;
    ...
    function computeColumnAligns(grid) {
    ...
    turndownService.addRule('tables', {
        ...
        replacement: function (_content, node) {
            ...
            const hasDataCells = rows.some(row => {
                const cells = Array.from(row.children).filter(cell => (cell.nodeName === 'TD' || cell.nodeName === 'TH') && !isHidden(cell));
            ...
            if (theadTr) {
                const idx = visibleTrs.indexOf(theadTr);
                if (idx >= 0)
                    headerRowIndex = idx;
            }
            else {
